...
<designspace format="3">
    <axes>
        <axis minimum="{SMIN}" default="{SMIN}" maximum="{SMAX}" name="Layer1-Scale" tag="LR1S"/>
        <axis minimum="{LMIN}" default="{LMIN}" maximum="{LMAX}" name="Layer1-X" tag="LR1X"/>
        <axis minimum="{LMIN}" default="{LMIN}" maximum="{LMAX}" name="Layer1-Y" tag="LR1Y"/>
        <axis minimum="{SMIN}" default="{SMIN}" maximum="{SMAX}" name="Layer2-Scale" tag="LR2S"/>
        <axis minimum="{LMIN}" default="{LMIN}" maximum="{LMAX}" name="Layer2-X" tag="LR2X"/>
        <axis minimum="{LMIN}" default="{LMIN}" maximum="{LMAX}" name="Layer2-Y" tag="LR2Y"/>
    </axes>
    <sources>
...
for s1 in ('S1min', 'S1max'):
    for x1 in ('X1min', 'X1max'):
        for y1 in ('Y1min', 'Y1max'):
            for s2 in ('S2min', 'S2max'):
                for x2 in ('X2min', 'X2max'):
                    for y2 in ('Y2min', 'Y2max'):
...
values = dict(S1min=SMIN, S1max=SMAX, X1min=LMIN, X1max=LMAX, Y1min=LMIN, Y1max=LMAX,
              S2min=SMIN, S2max=SMAX, X2min=LMIN, X2max=LMAX, Y2min=LMIN, Y2max=LMAX)
for s1 in ('S1min', 'S1max'):
    for x1 in ('X1min', 'X1max'):
        for y1 in ('Y1min', 'Y1max'):
            for s2 in ('S2min', 'S2max'):
                for x2 in ('X2min', 'X2max'):
                    for y2 in ('Y2min', 'Y2max'):
                        key = s1 + x1 + y1 + s2 + x2 + y2
                        if s1 == s2 == SMIN and x1 == y1 == x2 == y2 == LMIN:
                            info = '\t\t\t<info copy="1"/>\n'
                        else:
                            info = ''
                        ufoPath = DST_PATH % (s1, x1, y1, s2, x2, y2)
                        ...
                        colorGlyphs = {}
                        for glyphName in ('a', 'b', 'c', 'e', 'space'): #sorted(f.keys()):
                            #circleLayer1 = getPaintCircle(glyphName, values[s2], values[x2], values[y2], white)
                            if glyphName == 'e':
                                colorGlyphs[glyphName] = buildPixelGlyph('emsquare', pixelPositions(f, glyphName), pixelLayer1x, pixelLayer2x)
                            else:
